const FIELD_COPY = {
  ...
  sms: {
    title: "SMS",
    summary: (data) =>
      `${data.phone}${data.message ? ` • ${data.message}` : ""}`,
  },
  whatsapp: {
    title: "WhatsApp",
    summary: (data) =>
      ...
      return [
        {
          ...
          required: true,
          hint: "Use a valid URL. Missing https:// will be added automatically.",
        },
      ];
    case "text":
      return [
        {
          ...
          type: "textarea",
          value: values.text || "",
          placeholder: "Type any text, note, or message",
          required: true,
          hint: "Anything entered here becomes the QR payload.",
        },
      ];
    case "email":
      return [
      ...
      return [
        {
          ...
          required: true,
          hint: "Digits, spaces, parentheses, and + are allowed.",
        },
      ];
    case "sms":
      return [
...
export function collectFormValues(type, form) {
  ...
  switch (type) {
    ...
    case "email":
      ...
      data.subject = getValue("subject");
      data.body = getValue("body");
      break;
    case "phone":
    ...
    case "whatsapp":
      ...
      break;
    case "wifi":
      data.ssid = getValue("ssid");
      data.password = getValue("password");
      data.security = getValue("security") || "WPA";
      break;
    case "vcard":
      data.firstName = getValue("firstName");
      data.lastName = getValue("lastName");
      data.organization = getValue("organization");
...
export function validateForm(type, data) {
  ...
  switch (type) {
    ...
    case "text":
      if (!data.text) {
        errors.push("Plain text cannot be empty.");
      }
      break;
    case "email":
    ...
    case "phone":
      if (
        ...
        normalizePhone(data.phone || "").replace(/\D/g, "").length < 7
      ) {
    ...
    case "wifi":
      ...
      if (!data.password) {
        errors.push("Wi-Fi password is required.");
      }
      break;
    case "vcard":
      if (!data.firstName && !data.lastName) {
        errors.push(
          "At least a first or last name is required for a contact card.",
        );
      }
...
export function buildPayload(type, data) {
  switch (type) {
    ...
    case "sms": {
      const payload = normalizePhone(data.phone);
      return `SMSTO:${payload}:${data.message || ""}`;
    }
    case "whatsapp": {
    ...
    case "wifi":
      return `WIFI:T:${data.security || "WPA"};S:${escapeWifiValue(data.ssid)};P:${escapeWifiValue(data.password)};;`;
    case "vcard":
      return [
        "BEGIN:VCARD",
        "VERSION:3.0",
        ...
        data.phone
          ? `TEL;TYPE=WORK,VOICE:${escapeVcardValue(normalizePhone(data.phone))}`
          : null,
        data.email ? `EMAIL:${escapeVcardValue(data.email)}` : null,
        data.website
          ? `URL:${escapeVcardValue(ensureUrl(data.website))}`
          : null,
        data.address
          ? `ADR;TYPE=WORK:;;${escapeVcardValue(data.address)};;;;`
          : null,
